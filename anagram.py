...
parms={}
try:
	with open("anagram.conf","r") as file:
		for line in file:
...
try:
	with open(parms['wordFile'], 'r', encoding='UTF-8') as file:
		while line := file.readline():
			wordList.append(line.rstrip().lower())

	print("AnagramSolver: Imported",parms['wordFile'])
	
except FileNotFoundError:
	print("AnagramSolver: Couldn't find wordFile",parms['wordFile'])
	sys.exit()

except Exception:
	print("AnagramSolver: An error occurred reading wordFile",parms['wordFile'])
	sys.exit()
